  ...
  return lazypipe()
    ...
    .pipe(function() {
      return gulpif('*.scss', sass({
        outputStyle: 'nested', // libsass doesn't support expanded
        precision: 10,
        includePaths: ['.'],
...
gulp.task('styles', ['wiredep'], function() {
  var merged = merge();
    manifest.forEachDependency('css', function(dep) {
      var cssTaskInstance = cssTasks(dep.name);
        merged.add(gulp.src(dep.globs, {base: 'styles'})
          .pipe(plumber({errorHandler: onError}))
          .pipe(cssTaskInstance));
    });
    return merged
...
gulp.task('watch', function() {
    ...
    gulp.watch(path.source + 'imgs/**/*', ['images']);
    ...
    manifest.forEachDependency('css', function(dep) {
      var cssTaskInstance = cssTasks(dep.name);
        if (!enabled.failStyleTask) {
            cssTaskInstance.on('error', function(err) {
